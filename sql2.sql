...
(1,'Ariel Penayo','ariel_penayo@hotmail.com','4900d0a19b6894a4a514e9ff3afcc2c0',1,NULL),
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `usuario_asesorado_ext` AS select `usuario_asesorado`.`usuarioAsesoradoId` AS `usuarioAsesoradoId`,`usuario_asesorado`.`asesoradoId` AS `asesoradoId`,`asesorado`.`usuarioNombreApellido` AS `asesoradoNombreApellido`,`usuario_asesorado`.`asesoradorId` AS `asesoradorId`,`asesorador`.`usuarioNombreApellido` AS `asesoradorNombreApellido`,`usuario_asesorado`.`usuarioAsesoradoEstado` AS `usuarioAsesoradoEstado` from ((`usuario_asesorado` left join `usuario` `asesorado` on((`usuario_asesorado`.`asesoradoId` = `asesorado`.`usuarioId`))) left join `usuario` `asesorador` on((`usuario_asesorado`.`asesoradorId` = `asesorador`.`usuarioId`))) */;
